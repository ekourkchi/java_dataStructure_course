...
public class Sudoku {
  ...
  public static boolean fillSudoku (int [] [] sudoku)
  {
    ...
    for (i=0; i<9; i++)
      for (j=0; j<9; j++)
        if (sudoku[i][j] == 0) isFul = false;
    
    if (isFul) {   // Sudoku is full, check for solution validity
 
        return checkSudoku(sudoku, false);
    
    }
    else {  // Sudoku has at least one empty cell
        

       // Check for the first empty cell and stores its coordinate [i][j]
       n = 0;
        do {
           j = n%9;
           i = (n-j)/9;
           n++;
        }  while(sudoku[i][j] != 0 && n < 81);
        
        
        
        // For that empty cell, check all values for sudoku rule validity and solvability
        for (k=1; k<10; k++) {
            sudoku[i][j] = k;
            
            // if 'k' is a valid number
            if (checkSudoku(sudoku, false)) { 
               
               if(fillSudoku(sudoku))  // calling the method recursively to fill in the rest of Sudoku
                  return true;  
                  // sudoku is solved
               else 
                  sudoku[i][j] = 0; 
                  // if there is no solution for the given 'k', reset sudoku to its previous state
            }
            // else, if 'k' is an invalid number
            else {
                sudoku[i][j] = 0;
            }
        }
                   
        return false;  // if for none of 'k' numbers, sudoku is solvable, it returns false
    }
